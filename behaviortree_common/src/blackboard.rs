...
pub struct TypedBlackboard<T>(HashMap<String, T>);

impl<T> TypedBlackboard<T> {
    pub fn read_ref(&self, key: &str) -> Option<&T> {
        self.0.get(key)
    }

    pub fn read_ref_mut(&mut self, key: &String) -> Option<&mut T> {
        self.0.get_mut(key)
    }

    pub fn write(&mut self, key: String, data: T) {
...
impl<T> Input<T> {
    pub fn read_ref<'a>(&'a self, blackboard: &'a TypedBlackboard<T>) -> Option<&'a T> {
        match self {
...
impl Output {
    pub fn write<T>(&self, blackboard: &mut TypedBlackboard<T>, data: T) {
        match self {
